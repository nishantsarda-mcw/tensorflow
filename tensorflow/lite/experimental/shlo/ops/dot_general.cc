...
absl::Status PrepareImpl(DotGeneralOp& op, const Tensor& lhs, const Tensor& rhs,
                         Tensor& output) {
...
absl::Status Prepare(DotGeneralOp& op, const Tensor& lhs, const Tensor& rhs,
  ...
  if (lhs.IsQuantized()) {
    DISPATCH_BOOL_INT_FLOAT(
        PrepareImpl, lhs.quantized_per_tensor_element_type().ExpressedType(),
        op, lhs, rhs, output);
  } else {
    DISPATCH_BOOL_INT_FLOAT(PrepareImpl, lhs.StorageType(), op, lhs, rhs,
                            output);
  }
  return absl::OkStatus();
}
