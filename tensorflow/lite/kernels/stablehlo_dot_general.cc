...
TfLiteStatus CheckParameters(TfLiteContext* context, TfLiteNode* node,
  ...
  std::cout << "enter check parameters"
            << "\n";
...
TfLiteStatus EvalImpl(TfLiteContext* context, TfLiteNode* node,
  ...
  for (int batch = 0; batch < batch_size; ++batch) {
    ...
    if ((lhs->type == kTfLiteInt8 || lhs->type == kTfLiteInt16) &&
      ...
      std::vector<int32_t> flattened(eigen_dst.data(),
                                     eigen_dst.data() + eigen_dst.size());
      const int stride_output_data = batch * output_batch_size;

      for (int i = 0; i < flattened.size(); ++i) {
        int32_t total_scaled = MultiplyByQuantizedMultiplier(
            flattened[i], opdata.output_multiplier, opdata.output_shift);
        total_scaled += output->params.zero_point;
        total_scaled = std::max(total_scaled, opdata.output_activation_min);
        total_scaled = std::min(total_scaled, opdata.output_activation_max);
        output_data[stride_output_data + i] =
            static_cast<DataType>(total_scaled);
      }
    }
  }
...
void* Init(TfLiteContext* context, const char* options, size_t options_len) {
  DotGeneralData* opdata = new DotGeneralData();
  return opdata;
}
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  // per-tensor quantization params - maybe the setup separately for per-tensor
  // and per- channel is not required - the PopulateDotGeneralQuantizationParams
  // should do both!!
  ...
  if ((lhs->type == kTfLiteInt8 || lhs->type == kTfLiteInt16) &&
    ...
    TF_LITE_ENSURE_STATUS(GetQuantizedConvolutionMultipler(
        context, lhs, rhs, output, &real_multiplier));
    int exponent;
    QuantizeMultiplier(real_multiplier, &opdata.output_multiplier, &exponent);
    opdata.output_shift = exponent;

    if (lhs->type == kTfLiteInt8) {
      opdata.output_activation_min = std::numeric_limits<int8_t>::min();
      opdata.output_activation_max = std::numeric_limits<int8_t>::max();
    } else {
      opdata.output_activation_min = std::numeric_limits<int16_t>::min();
      opdata.output_activation_max = std::numeric_limits<int16_t>::max();
    }
